...
<? if ("2020-10-14" == "$DailyChallengesPageDate") { ?>
        <span class="challenges-date-arrow">&nbsp;</span>        
<? } else { ?>    
        <? if ($DailyChallengesLang == "") { ?>
        <a href="?date=<?php echo $yesterday->format('Y-m-d'); ?>">
        <? } else { ?>
        <a href="?date=<?php echo $yesterday->format('Y-m-d'); ?>&lang=<? echo $DailyChallengesLang; ?>">
        <? }?>
            <img class="challenges-date-arrow" src="assets/images/selection_left.png" alt="arrow">
        </a>  
<? } ?>        
    <?php echo $DailyChallengesDate; ?>
<? if ($currentDailyChallengesDate == "$DailyChallengesDate") { ?>        
<? } else { ?>
        <? if ($DailyChallengesLang == "") { ?>
        <a href="?date=<?php echo $tomorrow->format('Y-m-d'); ?>">
        <? } else { ?>
        <a href="?date=<?php echo $tomorrow->format('Y-m-d'); ?>&lang=<? echo $DailyChallengesLang; ?>">
        <? }?>
